...
/// '받은 제안' 상태와 비즈니스 로직을 관리
class SentProposalsViewModel extends PaginatedViewModelBase<Proposal> {
  ...
  Future<PaginatedResponse<Proposal>> fetchPage(int page) {
    return _useCase.getSentProposals(
  ...
  void setFilter(String filterKey) {
    if (_currentFilter != filterKey && filterOptions.containsKey(filterKey)) {
      ...
      // [수정] 부모 클래스의 loadData()를 호출하여 목록을 새로고침합니다.
      loadData();
    }
  }
}
