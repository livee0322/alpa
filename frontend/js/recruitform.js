/* /alpa/frontend/js/recruitform.js  (refactored, 2025-08-13)
 * - Cloudinary Signed Upload (기본: timestamp-only)
 * - folder 서명 사용하는 경우, CONFIG.USE_SIGNED_FOLDER = true 로 변경
 * - 파일 선택 시 미리보기/진행률
 * - 권한 가드(brand/admin), 에러 메시지 정리
 */

(function () {
  // ======= CONFIG =======
  const CONFIG = {
    USE_SIGNED_FOLDER: false, // 서버가 folder까지 서명하면 true
  ...
  async function getUploadSignature() {
    ...
    // 서버가 folder를 주더라도, CONFIG.USE_SIGNED_FOLDER=false면 사용하지 않음
  ...
  async function uploadToCloudinary(file) {
    ...
    if (CONFIG.USE_SIGNED_FOLDER && folder) {
      // 서버가 folder 포함해 서명한 경우에만 전송
  ...
  if (inputFile) {
    inputFile.addEventListener('change', async (e) => {
      ...
      try {
        ...
        // Cloudinary가 던지는 Invalid Signature 등 메시지도 그대로 노출
